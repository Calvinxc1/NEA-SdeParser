...
class Planet(_Base):
    ...
    ## Columns
    planet_id = Column(Integer(unsigned=True), primary_key=True, autoincrement=False)
    ...
    source = Column(Integer(unsigned=True))
    celestial_index = Column(Integer(unsigned=True))
    ...
    name = relationship(
        'Name',
        primaryjoin='Planet.planet_id==Name.item_id',
        foreign_keys='Name.item_id',
        uselist=False,
    )
    system = relationship('System', back_populates='planet')
    moon = relationship('Moon', back_populates='planet')
    belt = relationship('Belt', back_populates='planet')
    station = relationship('Station', back_populates='planet')
    type = relationship('Type')
    
    ## Aliased Columns
    planet_name = association_proxy('item_name', 'item_name')
    ...
    def sde_parse(cls, sde_record):
        sde_obj = cls(
            ...
            name_id=sde_record.get('planetNameID'),
            system_id=sde_record.get('solarSystemID'),
            source=sde_record.get('source'),
            celestial_index=sde_record.get('celestialIndex'),
